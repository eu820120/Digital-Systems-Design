...
#endif
{
    ...
    if ((lfsr%2) == 0) //check if lfsr[0] = 0
    {
    	IOWR_ALTERA_AVALON_PIO_DATA(DDS_INCREMENT_BASE, 0x56); //give value to dds_increment
    }
    if((lfsr%2) == 1) //check if lfsr[0] = 1
    {
    	IOWR_ALTERA_AVALON_PIO_DATA(DDS_INCREMENT_BASE, 0x1AE); //give value to dds_increment
    }
